...
- [Darwinian Evolver](https://github.com/imbue-ai/darwinian_evolver/)（Imbue）—— LLM 驱动的进化优化框架。用适应度 + 新颖度做选择，LLM 做变异，不行的淘汰。Mindclaw 的技能淘汰和晋升机制直接受其启发。
...
技能不是生出来就永远活着。这个思路来自 [Darwinian Evolver](https://github.com/imbue-ai/darwinian_evolver/)（Imbue）—— 一个用 LLM 驱动进化来优化代码和 prompt 的框架。它的核心机制是：维护一个"种群"，按适应度 + 新颖度加权采样，LLM 做变异，不行的淘汰，行的留下。在 ARC-AGI-2 上做到了 2-3 倍的性能提升。

Mindclaw 借鉴了这套自然选择的思路，但不是跑完整的进化循环，而是把它简化成技能层面的适应度评估和淘汰：
...
你需要一个已经跑起来的 [OpenClaw](https://github.com/nicepkg/openclaw) 实例。如果你还没装，先按 OpenClaw 的文档把网关跑通。

### 第 1 步：安装 Mindclaw

```bash
# 克隆项目
git clone https://github.com/chanokfortyzz/mindclaw.git
cd mindclaw
...
```bash
cp mindclaw.example.json mindclaw.json
```

编辑 `mindclaw.json`，按需开启模块。所有模块默认关闭，建议先只开 `traceLogger` 观察一下：
...
### 第 3 步：安装 Harness 插件（可选但推荐）

Harness 让 Mindclaw 能准确识别消息来自哪个渠道（微信/Discord/Telegram）：

```bash
node bin/install-harness.mjs ~/.openclaw
```

装完后重启 OpenClaw Gateway。
...
`--upstream` 指向你的 OpenClaw Gateway 地址。

### 第 5 步：修改 nginx 指向

把 nginx 从直接指向 OpenClaw 改为指向 Mindclaw：
...
```bash
# 查看 trace 日志，确认请求经过了 Mindclaw
tail -f logs/mindclaw-trace.jsonl
```

看到 JSONL 输出就说明管道在工作了。

### systemd 服务（生产环境）

```ini
# /etc/systemd/system/mindclaw-proxy.service
[Unit]
Description=Mindclaw AI Middleware Proxy
After=network.target

[Service]
Type=simple
WorkingDirectory=/opt/mindclaw
ExecStart=/usr/bin/node bin/mindclaw.mjs --port 7000 --upstream http://localhost:7005
Restart=always
RestartSec=2
...
```bash
systemctl enable --now mindclaw-proxy
```

### 回退

随时可以把 nginx 直接指回 OpenClaw Gateway（:7005），Mindclaw 就被完全绕过了。零风险。
